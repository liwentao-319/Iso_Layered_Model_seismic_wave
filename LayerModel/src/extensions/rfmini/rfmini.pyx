...
cdef extern int synrf_cwrap(
        ...
        double *fz, double *fr, double *rf, double *zrf)
...
def synrf(array  z_arr, array vp_arr, array vs_arr,
    ...
    cdef array fz_arr, fr_arr, rf_arr, zrf_arr
    cdef double *z
    cdef double *vp
    cdef double *vs
    cdef double *rh
    ...
    zrf_arr = numpy.zeros(nsamp,dtype=numpy.float64)
    nlay = len(z_arr) 
    z  = <double*>  z_arr.data
    ...
    zrf = <double*> zrf_arr.data 
    synrf_cwrap(nsamp, fsamp, tshift, p, a, nsv, sigma, waveno, nlay,
                z, vp, vs, rh, qp, qs, fz, fr, rf,zrf)

    return fz_arr,fr_arr,rf_arr,zrf_arr
