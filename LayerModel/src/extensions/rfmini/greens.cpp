...
compute_rf (int wave_type, Complex *cr, Complex *cz,
       ...
       double p, double vp0, double vs0, Complex *crf, Complex *czrf)
...
//     added czrf 2022/2/25 liwentao
{
	...
	for (j=0; j<nfreq; j++) {
		...
		if (denom<water) denom = water;
		crf[j] = cr[j]*conj(cz[j])/denom;
        czrf[j] = cz[j]*conj(cz[j])/denom;
...
tt=NULL; /* XXX */
	...
	// 		     laymin, laymax, pert);
	calcresp_core (model, wave, fref, nsamp, fsamp,
			     cz, cr, ct);
	...
	{	Complex crf [nfreq];
		...
		compute_rf (wave.type, cr, cz, nsamp, fsamp, water, tshift, a,
			wave.slowness, vp_top, vs_top, crf, czrf );
